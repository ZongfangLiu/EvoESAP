...
_FUSED_INVARIANCE_WARNED = False
...
class WeightMatchingPermuter(ExpertPermuter):
    ...
    def _fused_permute(
        ...
        Supported fused layouts:
        - gate_up_proj: (num_experts, hidden, 2 * expert_dim), down_proj: (num_experts, expert_dim, hidden)
        - gate_up_proj: (num_experts, 2 * expert_dim, hidden), down_proj: (num_experts, hidden, expert_dim)
        """
        ...
        up_shape = up_gate_proj.shape
        down_shape = down_proj.shape
        hidden_first = up_shape[1] == down_shape[2]
        hidden_last = up_shape[2] == down_shape[1]
        if hidden_first:
            expert_dim = down_shape[1]
            total_gate_up = up_shape[2]
        elif hidden_last:
            expert_dim = down_shape[2]
            total_gate_up = up_shape[1]
        else:
            raise ValueError(
                f"Unsupported fused expert layout: gate_up_proj={up_shape}, down_proj={down_shape}"
            )
        half = total_gate_up // 2
        if half <= 0:
            raise ValueError(
                f"Invalid fused expert layout: gate_up_proj={up_shape}, down_proj={down_shape}"
            )
        if half != expert_dim:
            logger.warning(
                "Fused expert dims mismatch (gate_up=%s, down=%s). "
                "Using gate_up_proj half=%d for permutation.",
                up_shape,
                down_shape,
                half,
            )
            expert_dim = half
        split_on_last = hidden_first

        if split_on_last:
            dom_gate = up_gate_proj[dom_expert_idx, :, :expert_dim]
            dom_up = up_gate_proj[dom_expert_idx, :, expert_dim : (2 * expert_dim)]
        ...
        for expert_idx in expert_indices:
            ...
            if split_on_last:
                ...
                this_up_proj = up_gate_proj[expert_idx, :, expert_dim : (2 * expert_dim)]
                ...
                down_cost = _weight_match_dist(this_down_proj.T, dom_down.T)
                cost_matrix = up_cost + gate_cost + down_cost
            ...
            if split_on_last:
                ...
                up_gate_proj[expert_idx, :, expert_dim : (2 * expert_dim)] = this_up_proj[:, permutation]
        ...
        try:
            ...
            global _FUSED_INVARIANCE_WARNED
            if not _FUSED_INVARIANCE_WARNED:
                logger.warning(
                    "Skipping fused permutation invariance check: %s", e
                )
                _FUSED_INVARIANCE_WARNED = True
