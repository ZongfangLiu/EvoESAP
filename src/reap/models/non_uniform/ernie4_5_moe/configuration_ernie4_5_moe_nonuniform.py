...
class Ernie4_5_MoeConfig(PreTrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # When constructing from a base config, user code may set `auto_map` later.
        if not hasattr(self, "auto_map"):
            self.auto_map = {}


class NonUniformErnie4_5_MoeConfig(Ernie4_5_MoeConfig):
    """
    Extends the upstream ERNIE-4.5 MoE config with per-layer expert counts.
    """
    ...
    def get_num_experts(self, layer_idx: int) -> int:
        ...
        return int(self.moe_num_experts)

    def to_dict(self):
        output = super().to_dict()
        output["num_experts_per_layer"] = self.num_experts_per_layer
        return output


__all__ = ["Ernie4_5_MoeConfig", "NonUniformErnie4_5_MoeConfig"]
